...
export default function Jobs({ jobs, currentUser, applications, isLoading = false }: JobsProps) {
  ...
  const Skeleton = () => <div className="animate-pulse rounded-2xl border border-brand-950/10 bg-white p-5"><div className="h-5 w-3/4 rounded bg-slate-100" /><div className="mt-3 h-4 w-1/2 rounded bg-slate-100" /><div className="mt-5 h-20 rounded bg-slate-100" /><div className="mt-5 h-11 rounded-full bg-slate-100" /></div>;

  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8" id="jobs-page-container">
      <header className="mb-8 flex flex-col justify-between gap-5 border-b border-brand-950/10 pb-7 md:flex-row md:items-end">
        <div>
          <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-[#3b82f6]"><BriefcaseBusiness className="h-4 w-4" />Local opportunities</p>
        ...
        {!currentUser && <div className="max-w-md rounded-2xl border border-brand-200 bg-brand-50 p-4"><div className="flex gap-3"><ShieldAlert className="mt-0.5 h-5 w-5 shrink-0 text-[#3b82f6]" /><div><p className="font-black text-slate-900">Hiring in Qardho?</p><p className="mt-1 text-sm text-slate-600">Sign in as an employer to publish a job.</p><Link to={PAGE_ROUTES.auth} className="mt-3 inline-flex min-h-11 items-center rounded-full bg-[#3b82f6] px-4 text-xs font-black text-white">Sign in to post</Link></div></div></div>}
      </header>

      <section className="mb-8 grid gap-3 rounded-2xl border border-brand-950/10 bg-white p-4 shadow-sm md:grid-cols-[1fr_18rem]" aria-label="Job search">
      ...
      ) : <div className="rounded-2xl border border-brand-950/10 bg-white py-16 text-center"><p className="font-bold text-slate-700">No jobs match this search.</p><button onClick={clearSearch} className="mt-3 min-h-11 rounded-full px-4 text-sm font-black text-[#2563eb] hover:bg-brand-50">Clear search</button></div>}
    </main>
  );
}
