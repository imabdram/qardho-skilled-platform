...
export default function PostJob({ currentUser, onPostJob, onNavigate }: PostJobProps) {
  ...
  const validate = () => {
    ...
    if (form.description.trim().length < 100) next.description = 'Job description must be at least 100 characters.';
    if (form.requirements.trim().length < 50) next.requirements = 'Requirements must be at least 50 characters.';
  ...
  const input = 'min-h-11 w-full rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold outline-none focus:border-[#008060] focus:ring-4 focus:ring-emerald-500/10';
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <header className="rounded-3xl bg-[#073f34] p-6 text-white sm:p-8"><p className="text-xs font-black uppercase tracking-[0.18em] text-[#b7f25c]">Employer workspace</p><h1 className="mt-3 text-3xl font-black sm:text-4xl">Post a job with clear expectations.</h1><p className="mt-3 max-w-2xl text-sm font-semibold leading-6 text-emerald-50/80">Complete the scope, requirements, pricing, and duration, then review everything before publishing.</p></header>

      <div className="mt-6 flex flex-wrap gap-2">{['Job details', 'Scope', 'Pricing', 'Review & publish'].map((step, index) => <span key={step} className={`rounded-full border px-3 py-2 text-xs font-black ${preview && index === 3 ? 'border-[#008060] bg-[#008060] text-white' : 'border-emerald-950/10 bg-white text-slate-600'}`}>{index + 1}. {step}</span>)}</div>

      <form onSubmit={submit} className="mt-6">
        {preview ? (
          <section className="rounded-3xl border border-emerald-950/10 bg-white p-5 shadow-sm sm:p-7">
            <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#008060]">Review before publishing</p><h2 className="mt-2 text-2xl font-black text-slate-950">{form.title}</h2><p className="mt-1 text-sm font-semibold text-slate-500">{form.category} - {form.workType} - {form.location}</p></div><Eye className="h-6 w-6 text-[#008060]" /></div>
            <div className="mt-6 grid gap-4 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><MapPin className="h-4 w-4 text-[#008060]" /><p className="mt-2 text-xs font-black uppercase text-slate-400">Location</p><p className="font-black">{form.location}</p></div><div className="rounded-2xl bg-slate-50 p-4"><DollarSign className="h-4 w-4 text-[#008060]" /><p className="mt-2 text-xs font-black uppercase text-slate-400">Pricing</p><p className="font-black">{derivedRate}</p></div><div className="rounded-2xl bg-slate-50 p-4"><Clock3 className="h-4 w-4 text-[#008060]" /><p className="mt-2 text-xs font-black uppercase text-slate-400">Duration</p><p className="font-black">{form.expectedDuration || 'Not specified'}</p></div></div>
