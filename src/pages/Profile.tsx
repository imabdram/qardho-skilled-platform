...
import {
  ...
  MessageSquare, Phone, Send, Share2, ShieldCheck, Star, UserCheck, UserRound, Wrench, Settings as SettingsIcon
} from 'lucide-react';
          ...
          <div className="space-y-6 lg:col-span-8">
            ...
            <section className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-xs">
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-black text-slate-900">Client Reviews & Feedback</h2>
                ...
                {userReviews.length > 0 ? (
                  userReviews.map((r) => (
                    <div key={r.id} className="rounded-xl border border-slate-100 bg-slate-50/50 p-4">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-slate-900">{r.employerName || 'Platform Client'}</span>
                        <div className="flex items-center gap-1 text-amber-500">
                          <Star className="h-3.5 w-3.5 fill-amber-500" />
                          <span className="text-xs font-bold text-slate-800">{r.rating}</span>
                        </div>
                      </div>
                      <p className="mt-2 text-xs font-medium text-slate-700 leading-relaxed">{r.comment}</p>
                      <p className="mt-2 text-[10px] text-slate-400">{r.createdAt ? new Date(r.createdAt).toLocaleDateString('en-GB') : 'Verified Review'}</p>
                    </div>
                  ))
                ) : (
                  <p className="text-xs italic text-slate-400 py-4">No reviews submitted yet.</p>
                )}
              </div>
            </section>
          </div>
