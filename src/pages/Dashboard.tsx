  ...
  const activeWorkCards: WorkItemData[] = useMemo(() => {
    ...
    return allWorkerProgressItems.map(item => {
      ...
      return {
        ...
        otherPartyAvatarUrl: getEmployerUser(job.employerId, job.employerName).avatarUrl,
  ...
  const applicationCards: WorkItemData[] = useMemo(() => {
    return myApplications.map(app => {
      ...
      return {
        ...
        otherPartyAvatarUrl: isWorker ? getEmployerUser(app.employerId, job?.employerName).avatarUrl : getWorker(app.applicantId)?.avatarUrl,
  ...
  const directOfferCards: WorkItemData[] = useMemo(() => {
    return myConnections.map(conn => {
      ...
      return {
        ...
        otherPartyAvatarUrl: isWorker ? getEmployerUser(conn.fromUserId, conn.fromUserName).avatarUrl : getWorker(conn.toUserId)?.avatarUrl,
        location: 'Qardho',
  ...
  const employerJobCards: WorkItemData[] = useMemo(() => {
    ...
    return myPostedJobs.map(job => {
      ...
      return {
        ...
        otherPartyAvatarUrl: getWorker(job.assignedWorkerId || acceptedApp?.applicantId)?.avatarUrl,
        location: job.location || 'Qardho',
        rate: job.rate,
