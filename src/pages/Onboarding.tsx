  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <section className="mt-6 rounded-3xl border border-emerald-950/10 bg-white p-5 shadow-sm sm:p-7">
        ...
        {step === 3 && <div><Check className="h-8 w-8 text-[#008060]" /><h2 className="mt-4 text-2xl font-black">Your setup is ready</h2><p className="mt-2 text-sm text-slate-600">Review the summary, then continue to the main page for your role.</p><div className="mt-6 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-slate-50 p-4"><span className="text-xs font-black uppercase text-slate-400">Role</span><p className="mt-1 font-black capitalize">{role}</p></div><div className="rounded-2xl bg-slate-50 p-4"><span className="text-xs font-black uppercase text-slate-400">Location</span><p className="mt-1 font-black">{location}</p></div>{role === 'worker' && <div className="rounded-2xl bg-slate-50 p-4"><span className="text-xs font-black uppercase text-slate-400">Skill</span><p className="mt-1 font-black">{skill}</p></div>}</div>{role === 'worker' && <div className="mt-7"><h3 className="flex items-center gap-2 text-lg font-black"><Search className="h-5 w-5 text-[#008060]" />Recommended jobs</h3><p className="mt-1 text-xs text-slate-500">Matched by your available profile details; if there is not enough information, recent jobs are shown.</p><div className="mt-3 grid gap-3 sm:grid-cols-3">{recommendedJobs.length ? recommendedJobs.map((job) => <div key={job.id} className="rounded-2xl border border-slate-100 p-4"><p className="font-black">{job.title}</p><p className="mt-1 text-xs text-slate-500">{job.location} - {job.rate}</p></div>) : <p className="text-sm text-slate-500">New jobs will appear here after setup.</p>}</div></div>}</div>}

        <div className="mt-8 flex flex-col-reverse gap-2 border-t border-slate-100 pt-5 sm:flex-row"><button type="button" onClick={() => { const next = Math.max(1, step - 1); persist(next); setStep(next); }} disabled={step === 1 || submitting} className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-full border border-slate-200 text-sm font-black disabled:opacity-40"><ArrowLeft className="h-4 w-4" />Back</button>{step < 3 ? <button type="button" onClick={goNext} className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-full bg-[#008060] text-sm font-black text-white">Continue<ArrowRight className="h-4 w-4" /></button> : <button type="button" onClick={finish} disabled={submitting} className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-full bg-[#008060] text-sm font-black text-white disabled:opacity-60">{submitting ? 'Saving...' : role === 'worker' ? 'Finish and browse jobs' : 'Finish and post a job'}<ArrowRight className="h-4 w-4" /></button>}</div>
      </section>
    </main>
  );
}
