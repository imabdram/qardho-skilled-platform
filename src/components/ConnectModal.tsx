    ...
    <div className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/60 backdrop-blur-sm sm:items-center sm:p-4" onMouseDown={(event) => { if (event.target === event.currentTarget && !isSubmitting) onClose(); }}>
      <section role="dialog" aria-modal="true" aria-labelledby="hire-dialog-title" className="max-h-[92dvh] w-full max-w-xl overflow-y-auto rounded-t-3xl bg-white shadow-2xl sm:rounded-3xl">
        ...
        <form onSubmit={submit} className="space-y-5 p-5 sm:p-6">
          ...
          {selectedJob && <div className="grid gap-3 rounded-2xl bg-emerald-50 p-4 sm:grid-cols-2"><div><span className="text-[10px] font-black uppercase text-emerald-700">Job title</span><p className="mt-1 text-sm font-black text-slate-900">{selectedJob.title}</p></div><div><span className="text-[10px] font-black uppercase text-emerald-700">Proposed price</span><p className="mt-1 flex items-center gap-1 text-sm font-black text-slate-900"><DollarSign className="h-4 w-4" />{selectedJob.rate}{selectedJob.pricingType ? ` / ${selectedJob.pricingType}` : ''}</p></div></div>}
          <div><label htmlFor="hire-message" className="mb-1.5 block text-sm font-black">Message</label><textarea id="hire-message" rows={5} value={message} onChange={(event) => setMessage(event.target.value)} className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm font-medium leading-6 outline-none focus:border-[#008060] focus:ring-4 focus:ring-emerald-500/10" /><p className="mt-1 text-xs text-slate-500">Your private number is shared only if the worker accepts.</p></div>
          ...
          <div className="flex flex-col-reverse gap-2 border-t border-slate-100 pt-4 sm:flex-row"><button type="button" onClick={onClose} disabled={isSubmitting} className="min-h-12 flex-1 rounded-full border border-slate-200 text-sm font-black">Cancel</button><button type="submit" disabled={isSubmitting} className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-full bg-[#008060] text-sm font-black text-white hover:bg-[#006b50] disabled:opacity-60"><Send className="h-4 w-4" />{isSubmitting ? 'Sending...' : 'Send job offer'}</button></div>
        </form>
      </section>
    </div>
  );
