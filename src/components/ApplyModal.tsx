    ...
    <div className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/60 p-0 backdrop-blur-sm sm:items-center sm:p-4" onMouseDown={(event) => { if (event.target === event.currentTarget && !isSubmitting) onClose(); }}>
      <section role="dialog" aria-modal="true" aria-labelledby="application-dialog-title" className="max-h-[92dvh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-white shadow-2xl sm:rounded-3xl">
        <header className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-slate-100 bg-white/95 px-5 py-4 backdrop-blur sm:px-6">
          <div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#008060]">Application review</p><h2 id="application-dialog-title" className="mt-1 text-xl font-black text-slate-950">{job.title}</h2><p className="mt-1 text-sm font-semibold text-slate-500">{job.employerName}</p></div>
        ...
        <form onSubmit={submit} className="space-y-5 p-5 sm:p-6">
          <div className="grid gap-3 rounded-2xl bg-emerald-50 p-4 sm:grid-cols-3">
            <div><span className="text-[10px] font-black uppercase text-emerald-700">Worker</span><p className="mt-1 text-sm font-black text-slate-900">{currentUser?.name}</p></div>
            <div><span className="text-[10px] font-black uppercase text-emerald-700">Relevant skill</span><p className="mt-1 text-sm font-black text-slate-900">{currentUser?.skill || 'General worker'}</p></div>
            <div><span className="text-[10px] font-black uppercase text-emerald-700">Job offer</span><p className="mt-1 text-sm font-black text-slate-900">{job.rate}</p></div>
          </div>

          <div><label htmlFor="application-message" className="mb-1.5 block text-sm font-black text-slate-800">Message</label><textarea id="application-message" rows={5} value={message} onChange={(event) => setMessage(event.target.value)} className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm font-medium leading-6 outline-none focus:border-[#008060] focus:ring-4 focus:ring-emerald-500/10" /><p className="mt-1 text-xs text-slate-500">Describe relevant experience, tools, and when you can start.</p></div>
          ...
          <div className="flex flex-col-reverse gap-2 border-t border-slate-100 pt-4 sm:flex-row"><button type="button" onClick={onClose} disabled={isSubmitting} className="min-h-12 flex-1 rounded-full border border-slate-200 px-4 text-sm font-black text-slate-700 hover:bg-slate-50">Cancel</button><button type="submit" disabled={isSubmitting} className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-full bg-[#008060] px-4 text-sm font-black text-white hover:bg-[#006b50] disabled:opacity-60"><Send className="h-4 w-4" />{isSubmitting ? 'Submitting...' : 'Submit application'}</button></div>
        </form>
      </section>
    </div>
  );
